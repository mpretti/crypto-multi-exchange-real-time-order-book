...
export function initChart() {
    if (!chartContainerEl) {
        console.error("Chart: chartContainerEl is not available");
        return;
    }

    // Ensure container has proper dimensions
    if (chartContainerEl.clientWidth === 0 || chartContainerEl.clientHeight === 0) {
        console.warn("Chart: Container has no dimensions, waiting for next frame");
        // Wait for next animation frame when container might have dimensions
        requestAnimationFrame(() => {
            if (chartContainerEl.clientWidth > 0 && chartContainerEl.clientHeight > 0) {
                initChart();
            } else {
                console.error("Chart: Container still has no dimensions after waiting");
            }
        });
        return;
    }
    ...
    try {
        ...
        // Validate the chart object was created successfully
        if (!newChart || typeof newChart.addCandlestickSeries !== 'function') {
            ...
            if (newChart && typeof newChart.remove === 'function') { 
                try { newChart.remove(); } catch (cleanupError) { 
                    console.warn("Chart: Error during cleanup of partially formed chart:", cleanupError); 
                } 
            }
            setChart(null);
            return;
        ...
        // Validate the series object was created successfully
        if (!newSeries || typeof newSeries.setData !== 'function' || typeof newSeries.update !== 'function') {
            console.error("Chart Error: chart.addCandlestickSeries did not return a valid series object.");
            setKlineSeries(null);
            if (newChart && typeof newChart.remove === 'function') { 
                try { newChart.remove(); } catch (cleanupError) { 
                    console.warn("Chart: Error during cleanup after failed series creation:", cleanupError); 
                } 
            }
            setChart(null);
            return;
        ...
        console.error("Chart: Exception during initialization:", e);
        setChart(null);
        setKlineSeries(null);
    }
}
